...
export function DeclareShipmentView({
  ...
  const [etape, setEtape] = useState<"saisie" | "photos" | "remise" | "protection">(
    "saisie",
  );
          ...
          {etape === "saisie" ? (
                      ...
                      <label className="flex cursor-pointer items-start gap-3">
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          <span className="block text-xs text-muted-foreground">
                            {offer.priceMajor} €{" "}
                            {offer.perPiece ? "par pièce" : "par kilo"}
                          </span>
                        </span>
                      </label>
            ...
            <>
              <CategoryPhotosStep
                ...
                value={
                  photos[choisies[photoIndex].categoryCode] ?? { files: [], note: "" }
                }
                onChange={(valeur) =>
                  setPhotos((courant) => ({
                    ...courant,
              ...
              <button
                ...
                disabled={
                  (photos[choisies[photoIndex].categoryCode]?.files.length ?? 0) <
                  MIN_PHOTOS
                }
                className="w-full rounded-xl bg-primary px-4 py-3 font-medium text-primary-foreground disabled:opacity-40"
              >
