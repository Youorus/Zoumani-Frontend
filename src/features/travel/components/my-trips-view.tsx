...
interface MyTripsViewProps {
  ...
  /**
   * Combien de colis en cours par voyage, indexé par identifiant.
   *
   * Un décompte et non la liste : la carte n'affiche qu'un nombre, et
   * charger les parcours entiers pour en compter trois serait du gâchis
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-6 px-4 sm:px-8">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <Link
          href="/compte/points"
          className="focus-ring rounded-lg text-sm font-bold text-primary"
        >
          Voir mes points
        </Link>
      </div>
      ...
      <ul className="grid gap-4 lg:grid-cols-2">
        {trips.map((trip) => (
          ...
          >
            ...
            {/* Les colis confiés, s'il y en a. Un lien et non un simple
                compteur : savoir qu'on transporte trois colis sans
                pouvoir les ouvrir n'avance à rien. */}
            {(parcelCounts[trip.id] ?? 0) > 0 ? (
              <Link
                href={`/compte/envois/suivi?voyage=${trip.id}` as Route}
                className="flex items-center gap-2 border-b border-border bg-primary/5 px-5 py-2.5 text-sm font-medium text-primary transition hover:bg-primary/10"
              >
                <Package className="size-4" aria-hidden />
                {parcelCounts[trip.id] === 1
                  ? "1 colis à transporter"
                  : `${parcelCounts[trip.id]} colis à transporter`}
              ...
              <div className="mt-5 flex items-center gap-3 rounded-xl bg-background p-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    {trip.segments
                      .map((segment) => `${segment.airlineCode}${segment.flightNumber}`)
                      .join(" · ")}
                  </p>
                </div>
              </div>
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 sm:px-8">
      <div className="relative overflow-hidden rounded-[2rem] bg-inverse-surface px-6 py-12 text-center text-inverse-foreground sm:px-12">
        <div
          className="absolute -right-16 -top-16 size-56 rounded-full bg-primary/20 blur-3xl"
          aria-hidden
        />
        <Sparkles className="relative mx-auto size-8 text-primary" aria-hidden />
        <p className="relative mt-4 text-xs font-bold uppercase tracking-[0.2em] text-primary">
          Votre prochain chapitre
        </p>
        <h1 className="relative mt-2 text-3xl font-semibold">
          Votre carnet de route vous attend.
        </h1>
